  ...
  */
  ...
  for (start, end; start < end; start += maxOptionsLength) {
    ...
    tasksInputsArray.push(
      Input({ label: ' ', blockId: `open-task-status-change-${start}` }).dispatchAction()
      .element(Elements.Checkboxes({ actionId: 'blockOpenTaskCheckboxClicked' }).options(holdingArray.map((task) => {
        const option = {
          text: `*${task.title}*`,
